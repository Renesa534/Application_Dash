...
def df_resample(df, maxlen=100):
    ...
    volume_df = vol_df.resample('{}ms'.format(int(rs))).sum()
...
# make a counter with blacklist words and empty word with some big value
stop_words.append('')
...
@app.callback(
    Output(component_id= 'recent-tweets-table', component_property= 'children'),
...
@app.callback(
    Output(component_id= 'sentiment-pie', component_property= 'figure'),
    [Input(component_id= 'sentiment_term', component_property= 'value')],
...
def update_graph_scatter(sentiment_term):
    try:
        ...
        return {'data': [data,data2],'layout' : pgo.Layout(xaxis=dict(range=[min(X),max(X)]),
...
@app.callback(
    Output(component_id= 'historical-graph', component_property= 'figure'),
    [Input(component_id= 'sentiment_term', component_property= 'value')],
...
def generate_size(value, smin, smax):
    ...
    final_size = (size_change*0.4) + 1
    return final_size*120

# SINCE A SINGLE FUNCTION CANNOT UPDATE MULTIPLE OUTPUTS...
@app.callback(
    Output(component_id= 'related-sentiment', component_property= 'children'),
    [Input(component_id= 'sentiment_term', component_property= 'value')],
...
@app.callback(
    Output(component_id= 'recent-trending', component_property= 'children'),
    [Input(component_id= 'sentiment_term', component_property= 'value')],
    [Input('related-update', 'n_intervals')]
)
